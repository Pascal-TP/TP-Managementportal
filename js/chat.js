...
function messageHtml(m){const mine=String(m.senderUid)===myUid();const a=m.attachment||null;let attachment="";if(a?.type==="image"&&a?.url){attachment=`<button class="chat-image-button" type="button" onclick="window.open('${esc(a.url)}','_blank','noopener')"><img class="chat-message-image" src="${esc(a.url)}" alt="${esc(a.fileName||"Chatbild")}" loading="lazy"></button>`;}else if(a?.type==="file"&&a?.url){attachment=`<a class="chat-file-card" href="${esc(a.url)}" target="_blank" rel="noopener" download><span class="chat-file-icon">${fileIcon(a.fileName,a.contentType)}</span><span class="chat-file-copy"><strong>${esc(a.fileName||"Datei")}</strong><small>${esc(formatBytes(a.size))} · Öffnen / herunterladen</small></span><span class="chat-file-download">⬇</span></a>`;}const text=m.text?`<div class="chat-message-text">${esc(m.text).replace(/\n/g,"<br>")}</div>`:"";return `<div class="chat-message-row ${mine?"mine":"theirs"}">${!mine?`<div class="chat-mini-avatar">${esc(initials(m.senderName))}</div>`:""}<div class="chat-bubble ${attachment?"has-image":""}"><div class="chat-message-meta">${!mine?`<strong>${esc(m.senderName||"Mitarbeiter")}</strong>`:""}<span>${fmt(m.createdAt)}</span></div>${attachment}${text}</div></div>`;}
