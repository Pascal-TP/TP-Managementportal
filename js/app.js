...
function bindNewDocumentForm() {
  ...
  document.querySelector("#remove-file-btn").onclick = () => { pendingUploadFile=null; input.value=""; document.querySelector("#file-selected").textContent="Noch keine Originaldatei ausgewählt"; zone.classList.remove("has-file"); document.querySelector("#remove-file-btn").hidden=true; };
  document.querySelector("#remove-pdf-btn").onclick = () => { pendingPdfFile=null; pdfInput.value=""; document.querySelector("#pdf-selected").textContent="Bei Workflow erforderlich."; pdfZone.classList.remove("has-file"); document.querySelector("#remove-pdf-btn").hidden=true; };
...
function setPendingFile(file) {
  ...
  document.querySelector("#drop-zone").classList.add("has-file"); document.querySelector("#remove-file-btn").hidden=false;
}
function setPendingPdfFile(file) {
  ...
  document.querySelector("#pdf-drop-zone").classList.add("has-file"); document.querySelector("#remove-pdf-btn").hidden=false;
}
async function submitNewDocument(e) {
  ...
  if (workflowEnabled && sourceIsPdf && !pendingPdfFile) return toast("Bei einem Workflow bitte die PDF-Lesefassung zusätzlich auswählen – auch wenn die Originaldatei bereits eine PDF ist.");
  try {
...
document.querySelector("#portal-info").onclick = () => {
  const helpFiles = {
    admin: "anleitungen/TP-Managementportal_Anleitung_Admin.pdf",
    supervisor: "anleitungen/TP-Managementportal_Anleitung_Vorgesetzte.pdf",
    employee: "anleitungen/TP-Managementportal_Anleitung_Mitarbeiter.pdf"
  };
  const file = helpFiles[currentProfile?.role] || helpFiles.employee;
